...
- [`TransferFunctions.kern_padding`](@ref) is used to determine the padding necessary to perform the full domain filtering of an
array.
...
- [`TransferFunctions.ind2sub`](@ref) is used in the [`FilteringMatrix`](@ref) to determine the "`kernel`" and "`parent`"
indices from the index to the `FilteringMatrix`.
