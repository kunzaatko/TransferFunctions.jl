...
BornWolf{N}(;λ, f, NA, n=float(4 // 3)) where {N} = BornWolf{N}(λ,NA,f,n)
BornWolf{N}(λ::Length, NA::Real, f::Real; kwargs...) where {N} = BornWolf{N}(; λ, f, NA, kwargs...)
BornWolf(args...; kwargs...) = BornWolf{3}(args...; kwargs...) # default to the 3D full model
...
refractive index `n`.
"""
function BornWolf{N}(λ::Length{A}, NA::Real, f::Real, n::Real) where {N,A<:Real}
